...
(
    uuid uuid PRIMARY KEY DEFAULT gen_random_uuid(),  -- номер счета (uuid пользователя)
    modified TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    balance integer CONSTRAINT not_negative_loyalty_value CHECK (balance >= 0) -- — текущий остаток на счёте
);
